...
module.exports = function(RED) {
    ...
    function AdjacentNode(n) {
        ...
        // Note: this adjacent doesn't do anything much - it will only send
        ...
        this.on('input', function (msg) {
            node.warn("Adjacent saw a payload: " + msg.payload);
            
         //var query = "MATCH (n:`Button`) RETURN n LIMIT 25";

         	//var query = 'MATCH (a:Apartment) WHERE a.name = "B3.4" RETURN a'
            var query = 'MATCH (a:Apartment {name:"'+ msg.payload +'"})-[:`ADJACENT`*]->(b)<-[:LIVES_IN]-(u:User) WHERE NOT b.name="' + msg.payload + '" RETURN DISTINCT u,b';
            ...
            db.query(query, function(err,results){
				...
				msg.payload = results.map(function (result) {
					...
					//return {user:result['u']['data'], apartment:result['b']['data']};
    ...
    // Node functions.
    RED.nodes.registerType("adjacent",AdjacentNode);
